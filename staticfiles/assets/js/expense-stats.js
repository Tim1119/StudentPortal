...
function getChartData(){
    fetch('/expenses/expense-summary-json/',{
        headers: {
    ...
    .then((res) => res.json())
    .then((results) => {
        if (Object.keys(results.expense_category_data).length > 0){
...
searchByDateRangeForm.addEventListener('submit',function(e){
    ...
    specificResult.classList.add('d-none')
    fetch('/expenses/search-within-date/',{
...
function changebar(){
    chartTitle.innerHTML = 'Bar Chart showing Expenses by Category'
  myChart.config.type = 'bar'
  myChart.update()
}
function changeline(){
    chartTitle.innerHTML = 'Line Chart showing Expenses by Category'
    myChart.config.type = 'line'
    myChart.update()
}
function changepie(){
    chartTitle.innerHTML = 'Pie Chart showing Expenses by Category'
    myChart.config.type = 'pie'
    myChart.update()
}
function changedoughnut(){
    chartTitle.innerHTML = 'Doughnut Chart showing Expenses by Category'
